...
void loop() {
  ...
  if ((buttonStateL == LOW) && (buttonStateR == LOW)) {
    if ((distance[1] < 10) && (distance[2] > distance[1]) && (distance[0] > distance[1])) {
      turnRight(5);
    } else if ((distance[2] < 10) && (distance[1] > distance[2]) && (distance[0] > distance[2])) {
      turnLeft(5);
    } else if ((distance[0] > 20) && (distance[1] > 10) && (distance[2] > 10)) {
      driveForward(1);
      //Serial.println(distance[0]);
    }
    //checkSensors();
